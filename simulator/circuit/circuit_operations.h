#ifndef __CIRCUIT_OPERATIONS__
#define __CIRCUIT_OPERATIONS__
...
// apply gate on State_Vector given a permutation map for the qubits
void apply_on_map(
		std::vector<comp> &SV,
...
// make qubit permutation map before applying gate
...
void Circuit::run() {
	...
	for(int i = 0; i < depth; ++i) {
		...
		make_permutation(permutation, qubits, layers[i]); // MAKE PERM MAP
		SqMatrix layer_array(0); 
		fill_gate(layer_array, layers[i]); // LAYER_MATRIX
		apply_on_map(state_vector, permutation, qubits, layer_array); // Multiply
	}
		...
		const std::vector<unsigned int> &qubs) 
{
	...
	// Like run(); but with only one layer containing the Obs *gate*
	std::vector<Application> layer = {Application(Obs, empty, qubs)};

	std::vector<unsigned int> permutation(qubits);
	make_permutation(permutation, qubits, layer); // make permutation
	SqMatrix layer_array(0);
	full_fill_gate(layer_array, layer, qubits); // make layer_matrix


	// Don't call apply_on_map since we don't need to unmap this time!!
